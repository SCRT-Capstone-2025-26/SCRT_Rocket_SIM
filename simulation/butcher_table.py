...
def butcher_tableaux(a, c, b, n=20, err=0.00001):
    ...
    for i in range(len(a)):
        ...
        csum[i] = lambda f, t, dt, u, asum=csum[i - 1]: sum(
            [a[i][j] * k[j](f, t, dt, u, asum) for j in range(i)]
        )
        if a[i][i] != 0:  # check if implicit
            k[i] = lambda f, t, dt, u, asum=csum[i]: iterate(
                lambda un: f(
                    t + c[i] * dt,
                    u[-1] + dt * (asum(f, t + c[i] * dt, dt, u) + a[i][i] * un),
                ),
                f(t, u[-1]),
            )
        else:  # it's explicit :)
            k[i] = lambda f, t, dt, u, asum=csum[i]: f(
                t + c[i] * dt, u[-1] + dt * asum(f, t, dt, u)
            )
    return lambda f, t, dt, u: (
        u[-1] + dt * sum([b[j] * k[j](f, t, dt, u) for j in range(len(b))])
    )
...
def butcher_table(a, b, c):
    def k(f, t, dt, u, i):
        if i >= 0:
            ...
            if a[i][i] != 0:
                return iterate(
                    lambda un: f(t + c[i] * dt, u[-1] + dt * (asum + a[i][i] * un)),
                    f(t + c[i] * dt, u[-1]),
                )
            else:
    ...
    return lambda f, t, dt, u: (
        u[-1] + dt * sum([b[j] * k(f, t, dt, u, j) for j in range(len(b))])
    )
